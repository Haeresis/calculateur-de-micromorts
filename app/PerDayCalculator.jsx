	...
	var PerDayCalculator = React.createClass({
		...
		daysChange: function() {
			var daysElapsed = this.refs.days.value.trim(),
				endDate = moment(this.refs.endDate.value.trim()),
				newStartDate = endDate.subtract(daysElapsed - 1,'days');
		...
		calculate : function(){
			var daysElapsed = this.refs.days.value.trim(),
				micromortsPerDay = this.props.micromorts / daysElapsed;
		...
		render: function() {
			var disabled = (this.props.micromorts == null || this.props.micromorts == Infinity) ? "disabled" : "",
				micromortsPerDayText = this.state.micromortsPerDay == null ? "[\xa0\xa0]" : this.state.micromortsPerDay.toFixed(2).replace(/(\d)(?=(\d{3})+\.\d\d$)/g,"$1,");
			return (
				<form className={"row intervalCalc " + disabled} onChange={this.onChange}>
					<h3 className="col-md-12">Micromorts par jour</h3>
					...
					<div className="col-md-4 row">
						<div className="row text-center">
							...
							<div className="col-md-12">
								...
								<label className="col-xs-12" htmlFor="days">Nombre de jours</label>
							</div>
						</div>
					</div>
					<h2 className="col-md-1 text-center">
